...
- **Authoritative server.** The server runs the simulation at a fixed 30 Hz tick: mechs, projectiles, AI robots, turrets, economy. Clients send only inputs (movement, aim, fire, build commands); the server simulates and broadcasts state snapshots at 15–30 Hz.
